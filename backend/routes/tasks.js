        ...
        try {
            ...
            for (let userId of assignees){
                const notification = new Notification({
                    user: userId, 
                    type: "TaskAssigned",
                    message: 'Youe have been assigneed to task: ${title}',
                });
                await notification.save();

                //Reql-time Notification via socket.io
            ...
            const activityLog = new ActivityLog({
                user: req.userId,
                action: "Created task",
                details: `Task "${title}" was created in board "${board.title}"`,
                relatedObject: task._id,
                objectType: "Task",
            });
